...
interface OrganizationDetails {
  ...
  services : { name: string, id : string, image : string, description : string , price :string , durationMin : string , durationMax : string, departmentId : string}[]
  address: string;
...
interface Service {
  name: string, id : string, image : string, description : string , price :string , durationMin : string , durationMax : string, departmentId : string
}
...
export default function OrganizationDetailsPage() {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR<OrganizationDetails>(`/api/organizations/${id}`, fetcher);
  ...
  return (
    ...
    <div className="relative h-64 container mx-auto max-w-6xl lg:mt-3 mt-0">
      <Image src={`${organization.imageCover}`} alt={organization.name} fill className="object-cover lg:rounded-2xl rounded-none" />
